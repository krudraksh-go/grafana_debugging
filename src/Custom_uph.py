...
class custom_IntervalThroughput:
    def get_merge_df(self, interval_start_date,interval_end_date, timeseriesdata,flow_type):
        ...
        if df_itemput.empty:
            return pd.DataFrame()
        df_itemput = self.utilfunction.flow_event(df_itemput,site_name=self.tenant_info["Name"])
    ...
    def get_item_pick(self, interval_start_date,interval_end_date, merge_df,flow_type):
        ...
        # path = f'butler_nav_incidents/{self.tenant_info["Name"]}_merge_df_{flow_type}.csv'
        # path1 = f'butler_nav_incidents/{self.tenant_info["Name"]}_group_df_{flow_type}.csv'
        # path2 = f'butler_nav_incidents/{self.tenant_info["Name"]}_df_itemput_{flow_type}.csv'
        # path = os.path.join(CONFFILES_DIR, path)
        # path1 = os.path.join(CONFFILES_DIR, path1)
        # path2 = os.path.join(CONFFILES_DIR, path2)
        # # path ="/home/ankush.j/Downloads/Carryover-butler_nav_incidents_v2_xxx.csv"
        # merge_df.to_csv(path, index=False, header=True)
        # group_df.to_csv(path1, index=False, header=True)
        # df_itemput.to_csv(path2, index=False, header=True)
        return group_df
    ...
    def get_interval_stop_date(self):
        q = f"select interval_stop from  interval_throughput_ecom_retail where time>='{self.start_date}' order by time desc limit 1"
        try:
        ...
        if not merge_df.empty:
            ...
            if not final_df.empty:
                ...
                if not final_df.empty:
                    ...
                    self.write_client.writepoints(final_df, "interval_throughput_ecom_retail", db_name=self.tenant_info["out_db_name"],
    ...
    def final_call(self, tenant_info, **kwargs):
        ...
        check_start_date = self.client.get_start_date("interval_throughput_ecom_retail", self.tenant_info)
        ...
        if self.tenant_info["Name"] == 'LLBean' or df.empty:
            self.end_date = datetime.now(timezone.utc)
            self.final_call1(self.end_date, **kwargs)
        else:
            try:
                daterange = self.client.get_datetime_interval3("interval_throughput_ecom_retail", '1h', self.tenant_info)
                if daterange.empty:
                    daterange = self.client.get_datetime_interval3("interval_throughput_ecom_retail", '15min', self.tenant_info)
                ...
                raise e
        return
    def final_call1(self, tenant_info, **kwargs):
        self.utilfunction = CommonFunction()
        self.start_date = self.client.get_start_date("interval_throughput_ecom_retail", self.tenant_info)
        self.start_date =pd.to_datetime(self.start_date).strftime("%Y-%m-%d %H:%M:%S")
        self.start_date=self.get_interval_stop_date()
        self.original_start_date = pd.to_datetime(self.start_date)+timedelta(minutes=1) #corner case
        ...
        if df_itempick.empty:
            filter = {"site": self.tenant_info['Name'], "table": "interval_throughput_ecom_retail"} 
...
with DAG(
    ...
    if os.environ.get('MULTI_TENANT_DAGS', 'false') == 'true':
        ...
        for tenant in csvReader:
            if tenant['Active'] == "Y" and tenant['custom_Interval_throughput'] == "Y"  and (tenant['is_production'] == "Y" or tenant['IsIndivisualInflux'] == "N"):
                final_task = PythonOperator(
                    task_id='custom_Interval_throughput_{}'.format(tenant['Name']),
                    provide_context=True,
                    python_callable=functools.partial(custom_IntervalThroughput().final_call,tenant_info={'tenant_info': tenant}),
        ...
        final_task = PythonOperator(
            task_id='custom_Interval_throughput',
            provide_context=True,
            python_callable=functools.partial(custom_IntervalThroughput().final_call,tenant_info={'tenant_info': tenant}),
