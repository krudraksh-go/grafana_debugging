...
class Client_setting:
    ...
    def expected_values_settings(self):
        ...
        if not df.empty:
            for index, row in df.iterrows():
                ...
                installation_id = final_df[final_df['field_name'] == 'installation_id']
                print(installation_id)
                if not installation_id.empty:
                    installation_id = installation_id.reset_index()
                    final_df['installation_id'] = installation_id['value'][0]

                final_df['non_peak'] = final_df['value'].str.split('|', expand=True)[0]
                final_df['peak'] = final_df['value'].str.split('|', expand=True)[1]
                final_df['abs_value'] = final_df['value'].str.split('|', expand=True)[2]
                final_df['non_peak'] = final_df['non_peak'].astype('str')
                final_df['peak'] = final_df['peak'].astype('str')
                final_df['abs_value'] = final_df['abs_value'].astype('str')
    ...
    def client_custom_settings(self):
    ...
    def final_call(self, tenant_info, **kwargs):
        ...
        df1 = self.expected_values_settings()
        df2= self.client_custom_settings()
        ...
        combine_df['value_float'] = combine_df['value'].apply(lambda x: -1.0 if type(x) is str else x)
        combine_df['value_float'] = combine_df['value_float'].astype(float,errors='ignore')
