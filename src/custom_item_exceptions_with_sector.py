...
## -----------------------------------------------------------------------------
class custom_item_exceptions_with_sector:
    def custom_item_exceptions_with_sector(self, tenant_info, **kwargs):
        ...
        check_start_date = self.client.get_start_date("item_exceptions_sector", self.tenant_info)
        ...
        if df.empty:
            ...
            self.custom_item_exceptions_with_sector1(self.end_date, **kwargs)
        else:
            try:
                daterange = self.client.get_datetime_interval3("item_exceptions_sector", '1h', self.tenant_info)
    ...
    def custom_item_exceptions_with_sector1(self, end_date, **kwargs):

        self.start_date = self.client.get_start_date("item_exceptions_sector", self.tenant_info)
        ...
        order_df = self.utilfunction.flow_event(df=order_df,site_name='sector')
        ...
        if not item_exceptions_df.empty and not order_df.empty:
            ...
            #df_final.rename(columns={'time_x': 'time'}, inplace=True)
            ...
            self.write_client.writepoints(df_final, "item_exceptions_sector", db_name=self.tenant_info["out_db_name"],
                                          tag_columns=['host', 'installation_id', 'pps_id'], dag_name=os.path.basename(__file__), site_name=self.tenant_info['Name'])
        else:
            filter = {"site": self.tenant_info['Name'], "table": "item_exceptions_sector"} 
...
## -----------------------------------------------------------------------------
with DAG(
    'custom_item_exceptions_with_sector',
    ...
    if os.environ.get('MULTI_TENANT_DAGS', 'false') == 'true':
        ...
        for tenant in csvReader:
            if tenant['Active'] == "Y" and tenant['custom_item_exceptions_with_sector'] == "Y"  and (tenant['is_production'] == "Y" or tenant['IsIndivisualInflux'] == "N"):
                custom_item_exceptions_with_sector_final_task = PythonOperator(
                    task_id='custom_item_exceptions_with_sector_final_{}'.format(tenant['Name']),
                    provide_context=True,
                    python_callable=functools.partial(custom_item_exceptions_with_sector().custom_item_exceptions_with_sector,tenant_info={'tenant_info': tenant}),
        ...
        custom_item_exceptions_with_sector_final_task = PythonOperator(
            task_id='custom_item_exceptions_with_sector_final',
            provide_context=True,
