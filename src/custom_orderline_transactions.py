...
## -----------------------------------------------------------------------------
class custom_Orderline_transactions:
    def custom_Orderline_transactions(self, tenant_info, **kwargs):
        ...
        check_start_date = self.client.get_start_date("orderline_transactions_ecom_retail", self.tenant_info)
        ...
        if df.empty:
            ...
            self.custom_Orderline_transactions1(self.end_date, **kwargs)
        else:
            try:
                daterange = self.client.get_datetime_interval3("orderline_transactions_ecom_retail", '1h', self.tenant_info)
                ...
                print(f"error:{e}")
                raise e
    def custom_Orderline_transactions1(self, end_date, **kwargs):
        self.start_date = self.client.get_start_date("orderline_transactions_ecom_retail", self.tenant_info)
        ...
        if not df.empty:
            ...
            df_transactions= df.groupby(['host','pps_id','order_id','installation_id', 'station_type','storage_type','ntile'], as_index=False).agg(Transactions=('item_id', 'size'), start_time=('time', 'min'), end_time=('time', 'max'),)
            ...
            df_order_flow = df.groupby(['order_id','installation_id','station_type','storage_type'], as_index=False).agg(order_flow=('order_flow', 'first'))
            df_final=pd.merge(df_orderline,df_transactions,on=['installation_id','pps_id','order_id','station_type','storage_type'],how='outer')
            ...
            self.write_client.writepoints(df_final, "orderline_transactions_ecom_retail", db_name=self.tenant_info["out_db_name"],
                                          tag_columns=['pps_id','station_type','storage_type'], dag_name=os.path.basename(__file__), site_name=self.tenant_info['Name'])
        else:
            filter = {"site": self.tenant_info['Name'], "table": "orderline_transactions_ecom_retail"} 
...
with DAG(
    ...
    if os.environ.get('MULTI_TENANT_DAGS', 'false') == 'true':
        ...
        for tenant in csvReader:
            if tenant['Active'] == "Y" and tenant['custom_Orderline_transactions'] == "Y"  and (tenant['is_production'] == "Y" or tenant['IsIndivisualInflux'] == "N"):
                custom_Orderline_transactions_final_task = PythonOperator(
                    task_id='custom_Orderline_transactions_final_{}'.format(tenant['Name']),
                    provide_context=True,
                    python_callable=functools.partial(custom_Orderline_transactions().custom_Orderline_transactions,tenant_info={'tenant_info': tenant}),
