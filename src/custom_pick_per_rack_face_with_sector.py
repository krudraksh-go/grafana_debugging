...
class custom_pick_per_rack_face_with_sector:
...
#        client.switch_database(db_name)
        ...
        check_start_date = self.client.get_start_date("picks_per_rack_face_sector", self.tenant_info)
        check_end_date = datetime.now(timezone.utc)
        check_start_date = pd.to_datetime(check_start_date)+timedelta(minutes=1)-timedelta(days=10) #corner case
        ...
        if df.empty:
            ...
            try:
                daterange = self.client.get_datetime_interval3("picks_per_rack_face_sector", '1h', self.tenant_info)
    ...
    def picks_per_rack_final1(self, end_date, **kwargs):
        self.start_date = self.client.get_start_date("picks_per_rack_face_sector", self.tenant_info)
        ...
        df = pd.DataFrame(self.read_client.query(q).get_points())
        if not df.empty:
            df = self.utilfunction.flow_event(df=df,site_name='sector')
            ...
            df['uom_quantity_int'] = df.apply(
                lambda x: int(x['uom_quantity']) if pd.isna(x['uom_quantity_int']) and not pd.isna(x['uom_quantity']) else x['uom_quantity_int'], axis=1)
            df['uom_quantity_int'] = df.apply(
            ...
            self.write_client.writepoints(df, "picks_per_rack_face_sector", db_name=self.tenant_info["out_db_name"],tag_columns=['pps_id','station_type','storage_type'], dag_name=os.path.basename(__file__), site_name=self.tenant_info['Name'])
        else:
            filter = {"site": self.tenant_info['Name'], "table": "picks_per_rack_face_sector"} 
...
with DAG(
    'Picks_per_rack_face_with_sector',
    ...
    if os.environ.get('MULTI_TENANT_DAGS', 'false') == 'true':
        ...
        for tenant in csvReader:
            if tenant['Active'] == "Y" and tenant['custom_picks_per_rack_face_with_sector'] == "Y"  and (tenant['is_production'] == "Y" or tenant['IsIndivisualInflux'] == "N"):
                final_task = PythonOperator(
                    task_id='picks_per_rack_final_{}'.format(tenant['Name']),
                    provide_context=True,
                    python_callable=functools.partial(custom_pick_per_rack_face_with_sector().picks_per_rack_final,tenant_info={'tenant_info': tenant}),
