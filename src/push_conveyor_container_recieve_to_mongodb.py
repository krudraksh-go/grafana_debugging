...
class ConveyorContainerRecieveToMongoDB:
    ...
    def push_data_to_mongodb(self, tenant_info, **kwargs):
        ...
        required_topics = [item for item in all_topics if item.endswith('conveyor-container-receive.requests')]
        topics = [item for item in required_topics if len(item.split('.')[1])<3]

        consumer = KafkaConsumer(
            *topics,
            bootstrap_servers=bootstrap_servers,
            group_id=site_name+'conveyor-container-receive-requests',
            auto_offset_reset='latest'
        )
        self.connection_string = MongoDbServer
        self.database_name = "kafka_data"
        self.collection_name = site_name + "conveyor_container_recieve_requests"
