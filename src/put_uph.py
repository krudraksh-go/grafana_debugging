...
class PutIntervalThroughput:
    def get_merge_df(self, interval_start_date,interval_end_date, timeseriesdata):
        ...
        q = f"select installation_id,pps_id,uom_quantity_int,uom_quantity,value,station_type,storagetype from item_put where time>'{interval_start_date}' and time<='{interval_end_date}'  order by time desc"
    ...
    def get_item_put(self, interval_start_date,interval_end_date, merge_df):
        interval_start_date1 = interval_start_date
        interval_start_date1 = interval_start_date1.strftime('%Y-%m-%dT%H:%M:%SZ')
        interval_end_date = interval_end_date.strftime('%Y-%m-%dT%H:%M:%SZ')
        q = f"select installation_id,user_id, pps_id,uom_quantity_int,uom_quantity,value ,station_type,storagetype from item_put where time>'{interval_start_date1}' and time<='{interval_end_date}' order by time desc"
        df_itemput = pd.DataFrame(self.read_client.query(q).get_points())
        df_itemput['time'] = pd.to_datetime(df_itemput['time'])
    ...
    def get_flow_event(self, interval_start_date,interval_end_date,merge_df):
        ...
        interval_start_date1 = interval_start_date1.strftime('%Y-%m-%dT%H:%M:%SZ')
        interval_end_date = interval_end_date.strftime('%Y-%m-%dT%H:%M:%SZ')
        q = f"select * from flow_events where time>'{interval_start_date1}'  and time<='{interval_end_date}' and process_mode ='put' order by time"
        ...
        if not df2_flowevents.empty:
            ...
            df2_flowevents['quantity_int'] = df2_flowevents['quantity_int'].apply(
                lambda x: int(x) if not pd.isna(x) else 0)
    ...
    def get_interval_stop_date(self):
        q = f"select interval_stop from  put_interval_throughput where time>='{self.start_date}' order by time desc limit 1"
        try:
    ...
    def final_call(self, tenant_info, **kwargs):
        ...
        check_start_date = self.client.get_start_date("put_interval_throughput", self.tenant_info)
        ...
        q = f"select *  from item_put where time>'{check_start_date}' and time<='{check_end_date}'  limit 1"
        ...
        if df_itemput.empty:
            ...
            try:
                daterange = self.client.get_datetime_interval3("put_interval_throughput", '1h', self.tenant_info)
    ...
    def final_call1(self, end_date, **kwargs):
      #  self.tenant_info = tenant_info['tenant_info']
        ...
        self.start_date = self.client.get_start_date("put_interval_throughput", self.tenant_info)
        ...
        q = f"select * from item_put where time>'{self.original_start_date.strftime('%Y-%m-%dT%H:%M:%SZ')}' and time<='{self.end_date.strftime('%Y-%m-%dT%H:%M:%SZ')}'  order by time desc limit 1"
        df_itemput = pd.DataFrame(self.read_client.query(q).get_points())
        if df_itemput.empty:
            filter = {"site": self.tenant_info['Name'], "table": "put_interval_throughput"} 
        ...
        if not merge_df.empty:
            df_item_put = self.get_item_put(init_date_frame["interval_start_date"][0],init_date_frame["interval_end_date"][0], merge_df)
            df2_flowevents = self.get_flow_event(init_date_frame["interval_start_date"][0],init_date_frame["interval_end_date"][0], merge_df)

            zero_data_frame=df_item_put[df_item_put['sum_qty']==0]
            non_zero_data_frame=df_item_put[df_item_put['sum_qty']>0]

            df1=sqldf("select b.pps_id,b.station_type, b.storage_type, b.installation_id, max(a.user_id) as user_d, b.interval_start,b.interval_end,b.ceil,min(a.time) as Min_time_wfm, b.Min_time,0 as sum_qty,0 as sum_value,b.flag as flag from df2_flowevents a inner join zero_data_frame b on   a.pps_id= b.pps_id and a.installation_id= b. installation_id and a.ceil= b.ceil and a.station_type= b.station_type and a.storage_type= b.storage_type group by b.installation_id, b.pps_id,b.interval_start,b.interval_end, b.flag,b.ceil,b.Min_time,b.station_type, b.storage_type" )
            if not df1.empty:
                df1["Min_time"] = df1[['Min_time','Min_time_wfm']].apply(lambda x: x['Min_time']if x['Min_time'] is not None   else x['Min_time_wfm'], axis=1)
            df2=sqldf("select b.pps_id,b.station_type, b.storage_type,max(b.user_id) as user_id, b.installation_id, b.interval_start,b.interval_end,b.ceil,min(b.Min_time) as Min_time_wfm, b.Min_time,0 as sum_qty,0 as sum_value,0  as flag from zero_data_frame b left join df2_flowevents a on   a.pps_id= b.pps_id and a.installation_id= b. installation_id and a.ceil= b.ceil and a.station_type= b.station_type and a.storage_type= b.storage_type where a.pps_id is null group by b.installation_id, b.pps_id,b.interval_start,b.interval_end,b.ceil,b.Min_time,b.station_type, b.storage_type")
            ...
            if not final_df.empty:
                ...
                if not final_df.empty:
                    ...
                    self.write_client.writepoints(final_df, "put_interval_throughput", db_name=self.tenant_info["out_db_name"],
                                                  tag_columns=['station_type','storage_type'], dag_name=os.path.basename(__file__), site_name=self.tenant_info['Name'])
        return None


with DAG(
    'put_interval_throughput',
    ...
    if os.environ.get('MULTI_TENANT_DAGS', 'false') == 'true':
        ...
        for tenant in csvReader:
            if tenant['Active'] == "Y" and tenant['put_interval_throughput'] == "Y"  and (tenant['is_production'] == "Y" or tenant['IsIndivisualInflux'] == "N"):
                final_task = PythonOperator(
                    task_id='put_interval_throughput_A_{}'.format(tenant['Name']),
                    provide_context=True,
                    python_callable=functools.partial(PutIntervalThroughput().final_call,tenant_info={'tenant_info': tenant}),
        ...
        final_task = PythonOperator(
            task_id='put_interval_throughput',
            provide_context=True,
            python_callable=functools.partial(PutIntervalThroughput().final_call,tenant_info={'tenant_info': tenant}),
            execution_timeout=timedelta(seconds=3600),
        )
