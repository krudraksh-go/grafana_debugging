...
class ReserveSlotAgaing:
    ...
    def reserve_slot_againg_final(self, tenant_info, **kwargs):
        ...
        if self.client.is_table_present(f'{rp_seven_days}.reserve_slots_ageing_ayx_latest'):
            table_name = f'{rp_seven_days}.reserve_slots_ageing_ayx_latest'
        else:
            table_name = 'reserve_slots_ageing_ayx_latest'
        ...
        df_4a.rename(
            ...
            inplace=True)
        # inner join
        df_joined = pd.merge(df_prev, df, how='inner', on=['unique_id', 'station_type', 'storage_type'])
        ...
        if not final_df.empty:
            ...
            summary_df = pd.DataFrame([{
                'installation_id': 'butler_demo_reserve',
                'time': cron_run_at,
            ...
            # except:
            #     pass
            self.write_client.writepoints(final_df, "reserve_slots_ageing_ayx_latest", db_name=self.tenant_info["out_db_name"],
...
with DAG(
        ...
        description='slot againg for reserve inventory',
    ...
    if os.environ.get('MULTI_TENANT_DAGS', 'false') == 'true':
        ...
        for tenant in csvReader:
            if tenant['Active'] == "Y" and tenant['Name'] == "HnMCanada":
                final_task = PythonOperator(
                    task_id='reserve_slot_againg_final_{}'.format(tenant['Name']),
                    provide_context=True,
                    python_callable=functools.partial(ReserveSlotAgaing().reserve_slot_againg_final,
        ...
        final_task = PythonOperator(
            task_id='reserve_slot_againg_final',
            provide_context=True,
            python_callable=functools.partial(ReserveSlotAgaing().reserve_slot_againg_final, tenant_info={'tenant_info': tenant}),
