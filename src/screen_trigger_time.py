...
## -----------------------------------------------------------------------------
class screen_trigger_time_summary:

    def apply_transaction_group(self, df):
        group = 0
        for x in df.index:
            if x > 0:
                if ((df['cur_trigger'][x] == 'add_pick_list') or
    ...
    def screen_trigger_time_summary(self, tenant_info, **kwargs):
        ...
        check_start_date = self.client.get_start_date("screen_trigger_time_summary_airflow", self.tenant_info)
        ...
        q = f"select *  from screen_trigger_time where time>'{check_start_date}' and time<='{check_end_date}' and (cur_trigger='pptl_press' ) and mode ='pick' limit 1"
        df_flow_transaction_events = pd.DataFrame(self.read_client.query(q).get_points())
        if df_flow_transaction_events.empty:
            self.end_date = datetime.now(timezone.utc)
            self.screen_trigger_time_summary1(self.end_date,df_flow_transaction_events.empty, **kwargs)
        else:
            try:
                daterange = self.client.get_datetime_interval3("screen_trigger_time_summary_airflow", '1h', self.tenant_info)
                if daterange.empty:
                    daterange = self.client.get_datetime_interval3("screen_trigger_time_summary_airflow", '5min', self.tenant_info)
    ...
    #                 x - 1] and df['rack_arrival_time'][x] == df['rack_arrival_time'][x - 1]:
    ...
    def screen_trigger_time_summary1(self, end_date,emptyflag, **kwargs):
        self.start_date = self.client.get_start_date("screen_trigger_time_summary_airflow", self.tenant_info)
        ...
        q = f"select * from screen_trigger_time where time>'{self.start_date}' and time<='{self.end_date}'  and cur_trigger='pptl_press' and mode ='pick' limit 1"
        df_main = pd.DataFrame(self.read_client.query(q).get_points())
        if df_main.empty:
            filter = {"site": self.tenant_info['Name'], "table": "screen_trigger_time_summary_airflow"}
            new_last_run = {"last_run": self.end_date}
            utilfunction.update_dag_last_run(filter, new_last_run)
            return

        q = f"select * from screen_trigger_time where time>'{temp_data}' and time<='{self.end_date}' and (((cur_trigger='pptl_press' or cur_trigger='first_item_pick' or cur_trigger='add_pick_list' or cur_trigger='carrier_barcode_scanned') and mode ='pick')) and cur_trigger!='last_trigger'   order by time"
        df_main = pd.DataFrame(self.read_client.query(q).get_points())
        print(q)
        ...
        if not df_main.empty:
            ...
            df_main['prev_trigger'] = df_main['cur_trigger'].shift(1)
            # df_main['prev_rack_id'] = df_main['rack_id'].shift(1)

            df_main = df_main[((df_main['prev_trigger'] != df_main['cur_trigger']) | (
                                       df_main['pps_id'] != df_main['prev_pps_id']) | (
                                       df_main['installation_id'] != df_main['prev_installation_id']) )]
            if not df_main.empty:
                l = ['prev_installation_id', 'prev_pps_id', 'prev_trigger']
                for col in l:
            ...
            df_main['transaction_start_time'] = df_main.apply(
                lambda x: x['time'] if x['cur_trigger'] == 'first_item_pick' else np.nan, axis=1)
            df_main['carrier_barcode_scanned_time'] = df_main.apply(
                lambda x: x['time'] if x['cur_trigger'] == 'carrier_barcode_scanned' else np.nan, axis=1)
            df_main['transaction_end_time'] = df_main.apply(
                lambda x: x['time'] if x['cur_trigger'] == 'pptl_press' else np.nan, axis=1)
            df_main['pptl_event_processed_time'] = df_main.apply(
                lambda x: x['time'] if (x['cur_trigger'] == 'pptl_press') else np.nan, axis=1)
            df_main['mode'] = df_main['mode'].apply(
                lambda x: x.replace('_mode', '') if not pd.isna(x) and len(x) > 4 else x)
            if 'cur_trigger' in df_main.columns:
                df_main['arrival_time'] = df_main.apply(
                    lambda x: x['time'] if x['cur_trigger'] == 'add_pick_list' else np.nan, axis=1)

            df_main['transaction_group'] = 0
            df_main = df_main[~((df_main['cur_trigger'] == 'add_pick_list') & (df_main['value'] < 6000))]
            ...
            if df_main.empty:
                return
            df_main = df_main.sort_values(
                by=['installation_id', 'pps_id', 'mode', 'transaction_group', 'time'],
                ascending=[True, True, True, True, True])
            df_main = utilfunction.reset_index(df_main)
            df_main['next_rack_arrival_time'] = df_main['rack_arrival_time'].shift(-1)
            ...
            df_main['next_rack_arrival_time'] = df_main.apply(lambda x:x['next_rack_arrival_time'] if x['installation_id']==x['installation_id_upper'] and x['pps_id']==x['pps_id_upper'] else np.nan, axis=1)
            ...
            df_main['time_diff_b_w_pptl_press_to_rack_arrival'] = df_main[
                'time_diff_b_w_pptl_press_to_rack_arrival'].apply(lambda x: 0 if x <= 0 or pd.isna(x) else x)
            df_main['lpn_scan_waiting_time'] = np.nan
            df_main = utilfunction.datediff_in_millisec(df_main, 'carrier_barcode_scanned_time',
                                                            'rack_arrival_time',
                                                            'lpn_scan_waiting_time')
            df_main['lpn_scan_waiting_time2'] = np.nan
            df_main = utilfunction.datediff_in_millisec(df_main, 'carrier_barcode_scanned_time',
                                                            'first_transaction_start_time',
                                                            'lpn_scan_waiting_time2')
            df_main['rack_arrival_to_first_transaction'] = np.nan
            df_main = utilfunction.datediff_in_millisec(df_main, 'first_transaction_start_time',
                                                            'rack_arrival_time',
                                                            'rack_arrival_to_first_transaction')
            #df_main['lpn_scan_waiting_time'] = df_main.apply(lambda x:x['lpn_scan_waiting_time'] if x['lpn_scan_waiting_time2'] < 0 else 0 ,axis=1 )
            # df_main['time_diff_b_w_pptl_press_to_rack_arrival'] = np.nan
            # df_main = utilfunction.datediff_in_millisec(df_main, 'rack_arrival_time',
            #                                             'pptl_event_processed_time',
            #                                             'time_diff_b_w_pptl_press_to_rack_arrival')
            ...
            # df_main = df_main.groupby(
            #     ['installation_id', 'pps_id', 'mode', 'ntile', 'station_type', 'storage_type'],
            #     as_index=False).agg(
            #     time=('time', 'min'),
            #     first_transaction_start_time=('first_transaction_start_time','min'),
            #     pptl_event_processed_time=('pptl_event_processed_time', 'max'),
            #     rack_arrival_time=('rack_arrival_time', 'max'),
            ...
            df_main['lpn_scan_waiting_time'] = df_main['lpn_scan_waiting_time'].astype(int, errors='ignore')
            df_main['lpn_scan_waiting_time2'] = df_main['lpn_scan_waiting_time2'].astype(int, errors='ignore')
            df_main['rack_arrival_to_first_transaction'] = df_main['rack_arrival_to_first_transaction'].astype(int, errors='ignore')
            df_main['pps_id'] = df_main['pps_id'].astype(str, errors='ignore')

            df_main["start"] = pd.to_datetime(self.start_date) - timedelta(minutes=5)
            ...
            if not df_main.empty and not emptyflag:
                l = ['rack_arrival_time','pps_id_upper', 'installation_id_upper', 'transaction_group','time_end','index','ntile','start','first_transaction_start_time_upper']
                for col in l:
                    if col in df_main.columns:
                        del df_main[col]
                df_main[['opertor_working_time', 'operator_waiting_time', 'lpn_scan_waiting_time','lpn_scan_waiting_time2','rack_arrival_to_first_transaction']] = df_main[['opertor_working_time', 'operator_waiting_time', 'lpn_scan_waiting_time','lpn_scan_waiting_time2','rack_arrival_to_first_transaction']].fillna(0)
                df_main['opertor_working_time'] = df_main['opertor_working_time'].apply(lambda x: 0 if x <= 0 or pd.isna(x) else x)
                df_main['rack_arrival_to_first_transaction'] = df_main['rack_arrival_to_first_transaction'].apply(
                    lambda x: 0 if x <= 0 or pd.isna(x) else int(x))
                df_main['actual_operator_working_time'] = df_main.apply(
                    lambda x: x['opertor_working_time'] + x['operator_waiting_time'] - x['time_diff_b_w_pptl_press_to_rack_arrival'], axis=1)
                df_main['actual_operator_working_time'] = df_main['actual_operator_working_time'].apply(
                    lambda x: 0 if x <= 0 or pd.isna(x) else x)
                df_main['time_diff_b_w_pptl_press_to_rack_arrival'] = df_main['time_diff_b_w_pptl_press_to_rack_arrival'].apply(
                    lambda x: 0 if x <= 0 or pd.isna(x) else x)
                ...
                df_main.carrier_barcode_scanned_time = df_main.carrier_barcode_scanned_time.apply(
                    lambda x: x.strftime('%Y-%m-%d %H:%M:%S') if not pd.isna(x) else x)
                ...
                df_main['carrier_barcode_scanned_time'] = df_main['carrier_barcode_scanned_time'].astype(str, errors='ignore')
                #df_main['actual_operator_working_time'] = df_main['actual_operator_working_time'].astype(str, errors='ignore')
                df_main['rack_arrival_to_first_transaction'] = df_main['rack_arrival_to_first_transaction'].astype(int, errors='ignore')
                df_main['actual_operator_working_time'] = df_main['actual_operator_working_time'].astype(int,errors='ignore')
                df_main['time_diff_b_w_pptl_press_to_rack_arrival'] = df_main['time_diff_b_w_pptl_press_to_rack_arrival'].astype(int,
                                                                                                         errors='ignore')
                df_main['lpn_scan_waiting_time2'] = df_main['lpn_scan_waiting_time2'].astype(float,errors='ignore')
                df_main['lpn_scan_waiting_time'] = df_main['lpn_scan_waiting_time'].astype(float,errors='ignore')
                ...
                self.write_client.writepoints(df_main, "screen_trigger_time_summary_airflow",
...
with DAG(
    ...
    if os.environ.get('MULTI_TENANT_DAGS', 'false') == 'true':
        ...
        for tenant in csvReader:
            if tenant['Active'] == "Y" and tenant['screen_trigger_time'] == "Y" and (tenant['is_production'] == "Y" or tenant['IsIndivisualInflux'] == "N"):
                try:

                    Client_setting_final_task2 = PythonOperator(
                        task_id='screen_trigger_time_summary_final_{}'.format(tenant['Name']),
                        provide_context=True,
                        python_callable=functools.partial(screen_trigger_time_summary().screen_trigger_time_summary,
        ...
        final_task = PythonOperator(
            task_id='screen_trigger_time_summary_final',
            provide_context=True,
            python_callable=functools.partial(screen_trigger_time_summary().screen_trigger_time_summary,
                                              tenant_info={'tenant_info': tenant}),
            execution_timeout=timedelta(seconds=3600),
