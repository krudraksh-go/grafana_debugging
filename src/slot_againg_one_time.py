...
class slot_againg_one_time:
    ...
    def slot_againg_one_time_final(self, tenant_info, **kwargs ):
        ...
        if self.tenant_info['Postgres_butler_user'] != "":
            ...
            df = pd.DataFrame(Orders,
                              columns=columnlist2)
            df = df.loc[(df['transaction_type'] == 'pick') | (df['transaction_type'] == 'put') | (
                        df['transaction_type'] == 'audit')]
...
#         final_df['last_transaction_time'] = current_transaction_time
        ...
        self.write_client.writepoints(final_df, "slots_ageing_ayx", db_name=self.tenant_info["out_db_name"],tag_columns=['rack_id'], dag_name=os.path.basename(__file__), site_name=self.tenant_info['Name'])
        if not final_df.empty:
            # try:
            #     self.client.query("drop measurement slots_ageing_ayx_latest")
            # except:
            #     pass
            self.write_client.writepoints(final_df, "slots_ageing_ayx_latest", db_name=self.tenant_info["out_db_name"],tag_columns=['rack_id'], dag_name=os.path.basename(__file__), site_name=self.tenant_info['Name'],retention_policy =rp_seven_days)
...
with DAG(
    'slot_againg_one_time',
    default_args = CommonFunction().get_default_args_for_dag(),
    description = 'cslot_againg_one_time',
    ...
    if os.environ.get('MULTI_TENANT_DAGS', 'false') == 'true':
        ...
        for tenant in csvReader:
            if tenant['Active'] == "Y" and tenant['slot_againg_one_time'] == "Y"   and (tenant['is_production'] == "Y" or tenant['IsIndivisualInflux'] == "Y"):
                final_task = PythonOperator(
                    task_id='slot_againg_one_time_final_{}'.format(tenant['Name']),
                    provide_context=True,
                    python_callable=functools.partial(slot_againg_one_time().slot_againg_one_time_final,tenant_info={'tenant_info': tenant}),
                    execution_timeout=timedelta(seconds=7200),
                )
    else:
        tenant = CommonFunction().get_tenant_info()
        final_task = PythonOperator(
            task_id='slot_againg_one_time_final',
            provide_context=True,
            python_callable=functools.partial(slot_againg_one_time().slot_againg_one_time_final,tenant_info={'tenant_info': tenant}),
