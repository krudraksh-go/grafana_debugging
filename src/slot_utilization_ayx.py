...
class slot_utilzation:
    ...
    def slot_utilzation_calc(self, tenant_info, end_date, **kwargs):
        ...
        q_cnt = f"select count(cron_ran_at) from {table_name}  where time>= '{start_date}' order by time desc"
        ...
        while offset_val < q_cnt:
            q = f"select * from {table_name} where time>= '{start_date}'order by time desc limit {data_limit} offset {offset_val}"
        ...
        if not df.empty:
            ...
            dfb = dfa.groupby('key', as_index=False).agg(consolidated_total_rack=('consolidated_total_rack', 'sum'),
                                                         total_slots=('total_slots', 'sum'), total_racks=('total_racks', 'sum'), )
            dfb = dfb.reset_index()
            ...
            dfb = dfb.rename(columns={'key': 'racktype', 'consolidated_total_rack': 'total_rack_faces', 'total_racks': 'total_racks'})
            df3 = dfb
            ...
            dfb = dfb.rename(columns={"first_consolidated_total_rack": "total_rack_faces"})
            ...
            df3['avg_rack_face_utilization'] = df3['avg_rack_face_utilization'] / df3['total_rack_faces']
            ...
            final_df['total_rack_faces'] = final_df['total_rack_faces'].astype(float)
            if 'level_0' in final_df:
                del final_df['level_0']
            self.write_client = Write_InfluxData(host=self.tenant_info["write_influx_ip"],
                                                 port=self.tenant_info["write_influx_port"])
            self.write_client.writepoints(final_df, "slots_utilization_ayx", db_name=self.tenant_info["out_db_name"],
...
with DAG(
    ...
    if os.environ.get('MULTI_TENANT_DAGS', 'false') == 'true':
        ...
        for tenant in csvReader:
            if tenant['Active'] == "Y" and tenant['slot_utilzation'] == "Y"   and (tenant['is_production'] == "Y" or tenant['IsIndivisualInflux'] == "Y"):
